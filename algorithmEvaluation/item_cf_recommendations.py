            ...
            if item1 != item2:
                ...
                for user_id, interaction1 in users_dict1.items():
                    if user_id in user_ids_set2:
                        ...
                        # 计算模长
                        norm1 += (0.5 * rating1 ** 2 + 0.3 * int(favorite1)+ 0.2 * int(browse1))
                        norm2 += (0.5 * rating2 ** 2 + 0.3 * int(favorite2)+ 0.2 * int(browse2))

                cos_sim = dot_product / (math.sqrt(norm1) * math.sqrt(norm2)) if norm1 > 0 and norm2 > 0 else 0
                ITEM_SIMILARITY[item1][item2] = round(cos_sim, 4)

    return ITEM_SIMILARITY

def recommend_sights(user_id, k):
    ...
    for similar_item, recommendation_value in recommendations.items():
        # 删除旧的推荐度
        UserRecommendationValuesTest.objects.filter(
            user_id=user_id,
            sight_id=similar_item
        ).delete()

        # 插入新的推荐度
        UserRecommendationValuesTest.objects.create(
            user_id=user_id,
            sight_id=similar_item,
